...
type PatchMeetupIDAttendeeURL struct {
	ID string
...
func (o *PatchMeetupIDAttendeeURL) Build() (*url.URL, error) {
	...
	var _path = "/meetup/{id}/attendee"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on PatchMeetupIDAttendeeURL")
	}
...
func (o *PatchMeetupIDAttendeeURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PatchMeetupIDAttendeeURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PatchMeetupIDAttendeeURL")
	}
